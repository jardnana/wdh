...
class Currency extends CI_Controller {
	...
	function index(){
	  $data['currency'] = $this->General_Model->get_currency_list(); 
	...
	function add_currency(){
		if(count($_POST) > 0){
			...
			$this->form_validation->set_rules('currency_value','Currency Value','required');
			if($this->form_validation->run()==TRUE){			  
				$Query = $this->General_Model->get_currency_list($id = '', $_POST['currency_code']);
				if(!empty($Query)){
					$currency['status'] = "Currency Code Already Existed.";
					$this->load->view('currency/add_currency',$currency);
				} else {
				$statis	= $this->Currency_Model->add_currency_details($_POST);
					redirect('currency','refresh');
				}	
			} else {
				$currency 					= $this->General_Model->get_home_page_settings();
				$this->load->view('currency/add_currency',$currency);
			}		
		} else {
			$currency 					= $this->General_Model->get_home_page_settings();
	...
	function edit_currency($id){
		...
		if($id != ''){ 
			$data['currency'] = $this->General_Model->get_currency_list($id);
		...
		if($id != ''){
			...
			if($this->form_validation->run()==TRUE){		
				$data			= array(
									...
									'country_code' 	=> $_REQUEST['currency_symbol'],
									'currency_name'	=> $_REQUEST['currency_name'],
									'value'			=> $_REQUEST['currency_value']
								);
				$this->Currency_Model->update_currency_details($id,$data);
			} else {
				redirect('currency/edit_currency/'.$id, 'refresh');
			}
		}
		redirect('currency','refresh');
	}
	function delete_currency($id){
		$id = json_decode(base64_decode($id));
		if($id != ''){ 
			$this->Currency_Model->delete_currency_details($id);
		}
		redirect('currency','refresh');
	} 
	...
	function ConverCurrency($from_code, $to_code) {
		...
		$from_Currency 		= urlencode($from_code);	
		$to_Currency 		= urlencode($to_code);	
		$currency_text 		= $from_Currency."_".$to_Currency;
		...
		if(isset($values->$currency_text->val) && isset($values->$currency_text->val) !=''){
			$converted_currency = preg_replace("/[^0-9\.]/", null, $values->$currency_text->val);			
		}else{
	...
	function currencyUpdate(){	
		...
		for($i=0; $i< count($country_currencies);$i++){
			$restricAr	= array();
			if(true){
				$value[$i] = round($this->ConverCurrency_Layer(BASE_CURRENCY,$country_currencies[$i]->currency_code),4);
				echo '<pre/>';print_r($currency_values);exit;
				if($value[$i] > 0){
					$data	   = array('value' => $value[$i]);
					$this->db->where('cur_id',$country_currencies[$i]->cur_id);
	...
	function ConverCurrency1($from_code, $to_code) {
		ini_set('max_execution_time', 600);
